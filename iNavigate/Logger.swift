...
//  iNavigate
//
//  Created by Giovanni Fusco on 11/27/19.
...
class Logger{
    ...
    func logData(currentARFrame: ARFrame, scaledFrame: UIImage){
        //print("SAMPLING: \(samplingTime)" )
        if (scaledFrame.size.height > 0){
            if lastTimeUpdated == 0 || currentARFrame.timestamp - lastTimeUpdated > self.samplingTime{
                
                lastTimeUpdated = currentARFrame.timestamp
                let imageFilename = "\(lastTimeUpdated)"+".jpg"
